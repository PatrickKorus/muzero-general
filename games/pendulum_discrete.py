...
class MuZeroConfig:
    def __init__(self):
        ...
        self.action_space = [i for i in range(5)]  # Fixed list of all possible actions. You should only edit the length
        ...
        self.training_device = "cpu" # "cuda" if torch.cuda.is_available() else "cpu"  # Train on GPU if available
...
class DiscreteActionWrapper(gym.ActionWrapper):
    def __init__(self, env, num_actions=5):
        ...
        self.action_equivalents = numpy.array([self.original_action_space.low,
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = DiscreteActionWrapper(gym.make("Pendulum-v0"))
        if seed is not None:
            self.env.seed(seed)
